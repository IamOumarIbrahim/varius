# ⚔️ Cave Survivor: Tsushima Stances
...
graph TD
    ...
    Engine --> UI["ui.py (UIManager: Scrollable Lobby, Settings & Toolbar HUD)"]
    ...
    UI --> Char["characters.py (69 Pop-Culture DB characters)"]
...
* **Use Selected Item / Attack**: **`F` Key**
  * Select Slot 1 (Pickaxe) $\rightarrow$ Aim mouse at block $\rightarrow$ Press `F` to mine.
  * Select Slot 2 (Katana/Unique weapon) $\rightarrow$ Press `F` to swing.
  * Select Slot 3 (Block) $\rightarrow$ Aim mouse $\rightarrow$ Press `F` to build a dirt tile (requires iron).
